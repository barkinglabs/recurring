...
class GiftCard extends RecurlyData {
  ...
  get href() {
    return `${this.constructor.ENDPOINT}/${this.redemption_code}`
  }

  redeem(options, callback) {
    debug('calling redeem with options: %o', options)
    if (!options.account_code) {
      throw (new Error('gift card redemption options must include "account_code" parameter'))
    }

    const body = data2xml('recipient_account', options)
    ...
    this.post(href, body, (err, response, payload) => {
      ...
      const giftCard = this._recurring.GiftCard()
      giftCard.inflate(payload)
