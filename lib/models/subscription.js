...
class Subscription extends RecurlyData {
  ...
  preview(options, callback) {
    ...
    const body = data2xml(Subscription.SINGULAR, options)
    const href = `${Subscription.ENDPOINT}/preview`
  ...
  postpone({ nextRenewal, bulk }, callback) {
    if (!this.id) {
  ...
  invoice(callback) {
    if (!this._resources.invoice) {
      throw new Error('No invoice resource available')
    }

    const invoice = this._recurring.Invoice()
    invoice.href = this._resources.invoice
