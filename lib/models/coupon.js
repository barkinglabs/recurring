...
class Coupon extends RecurlyData {
  ...
  generate(options, callback) {
    debug('calling generate with options: %o', options)
    if (!options.number_of_unique_codes) {
      throw (new Error('coupon generation options must include "number_of_unique_codes" parameter'))
    }

    const body = data2xml(Coupon.SINGULAR, options)
    const href = `${this.href}/generate`;

    this.post(href, body, { noParse: true }, (err, response, payload) => {
      const error = handleRecurlyError(err, response, payload, [ 200, 201 ])
      if (error) {
        return callback(error)
      }
      ...
      if (response.statusCode !== 201) {
        return callback(new RecurlyError(`Unexpected response status code: ${response.statusCode}`))
      }

      if (!response.headers.location) {
        return callback(new RecurlyError(`Expected 'Location' header`))
      }
